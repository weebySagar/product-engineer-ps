...
- **store** (`src/store.js`) — the only MongoDB touchpoint. A Mongoose `Memory` model with `topic`, `subject`, `content`, `kind`, `source`, `state` (`active` | `superseded` | `deleted`), and ObjectId links `supersedes` / `supersededBy` / `conflictsWith`. Provides `findActiveMemories`, `createMemory`, `supersede`, `softDelete`, `addConflict`.
